 ...
 * @since      Moodle 3.0
 */
class mod_mediamanager_external extends external_api {
    ...
    public static function view_resource($resourceid) {
        ...
        $resource = $DB->get_record('mediamanager', array('id' => $params['resourceid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($resource, 'mediamanager');
    ...
    public static function get_resources_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            foreach ($resources as $resource) {
                ...
                helper_for_get_mods_by_courses::format_name_and_intro($resource, 'mod_mediamanager');
                $resource->contentfiles = external_util::get_area_files($context->id, 'mod_mediamanager', 'content');
