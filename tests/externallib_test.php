...
/**
 * External mod_mediamanager functions unit tests
 *
...
class externallib_test extends externallib_advanced_testcase {
    ...
    /**
     * Test view_mediamanager
     */
    public function test_view_mediamanager() {
        global $DB;
        ...
        $resource = $this->getDataGenerator()->create_module('mediamanager', array('course' => $course->id));
        $context = \context_module::instance($resource->cmid);
        $cm = get_coursemodule_from_instance('mediamanager', $resource->id);

        // Test invalid instance id.
        try {
            mod_mediamanager_external::view_resource(0);
            $this->fail('Exception expected due to invalid mod_mediamanager instance id.');
        ...
        $this->setUser($user);
        try {
            mod_mediamanager_external::view_resource($resource->id);
        ...
        $result = mod_mediamanager_external::view_resource($resource->id);
        $result = \external_api::clean_returnvalue(mod_mediamanager_external::view_resource_returns(), $result);
        ...
        $this->assertInstanceOf('\mod_mediamanager\event\course_module_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $moodleurl = new \moodle_url('/mod/mediamanager/view.php', array('id' => $cm->id));
        ...
        assign_capability('mod/mediamanager:view', CAP_PROHIBIT, $studentrole->id, $context->id);
    ...
    /**
     * Test test_mod_mediamanager_get_resources_by_courses
     */
    public function test_mod_mediamanager_get_resources_by_courses() {
        global $DB;
        ...
        $resource1 = self::getDataGenerator()->create_module('mediamanager', $record);

        // Second resource.
        $record = new \stdClass();
        $record->course = $course2->id;
        $resource2 = self::getDataGenerator()->create_module('mediamanager', $record);
        ...
        $returndescription = mod_mediamanager_external::get_resources_by_courses_returns();
        ...
        $result = mod_mediamanager_external::get_resources_by_courses(array($course2->id, $course1->id));
        ...
        $result = mod_mediamanager_external::get_resources_by_courses();
        $result = \external_api::clean_returnvalue($returndescription, $result);
        ...
        $result = mod_mediamanager_external::get_resources_by_courses(array($course2->id));
